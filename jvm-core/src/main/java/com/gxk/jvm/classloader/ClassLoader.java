...
public class ClassLoader {
  ...
  public void loadPrimitiveClass(String name) {
    KClass cache = MethodArea.findClass(name);
    if (cache != null) {
      return;
    }
    KClass cls = new KClass(1, name, this);
    Long metaCls = MethodArea.findClass("java/lang/Class").newObject();
    cls.setRuntimeClass(metaCls);
    Heap.load(metaCls).setMetaClass(cls);
  ...
  public KClass doLoadClass(String name) {
    ...
    if (MethodArea.findClass("java/lang/Class") != null) {
      Long rcs = MethodArea.findClass("java/lang/Class").newObject();
      kClass.setRuntimeClass(rcs);
      Heap.load(rcs).setMetaClass(kClass);
    }
  ...
  public KClass doLoadClass(String name, ClassFile classFile) {
    ...
    for (KField it : fields) {
      switch (it.descriptor) {
        ...
        case "S":
        case "I":
          it.setVal(new Slot[]{new Slot(0, Slot.INT)});
          break;
        case "F":
          it.setVal(new Slot[]{new Slot(0, Slot.FLOAT)});
          break;
        case "D":
          it.setVal(new Slot[]{new Slot(0, Slot.DOUBLE_HIGH), new Slot(0, Slot.DOUBLE_LOW)});
          break;
        case "J":
          it.setVal(new Slot[]{new Slot(0, Slot.LONG_HIGH), new Slot(0, Slot.LONG_LOW)});
          break;
        default:
          it.setVal(new Slot[]{new Slot(null)}); // null
          break;
      }
    }
